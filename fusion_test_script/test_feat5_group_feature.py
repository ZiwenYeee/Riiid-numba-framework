...
def valid_feats5(valid_gp, rolling_gp):
    ...
    for i in range(len(valid_gp)):
        ...
        if tmp_g[0,0] in rolling_gp:
            ...
            if tmp_rolling.shape[0] > 10:
                tmp_rolling = tmp_rolling[-10:,:]
...
def test_feats5_wrap(valid, rolling_gp):
...
def rolling_df_update(rolling_gp, test_gp):
    for gp in test_gp:
        if gp[0][0] in rolling_gp:
            rolling_gp[gp[0][0]] = np.concatenate([rolling_gp[gp[0][0]], gp])
        else:
            rolling_gp[gp[0][0]] = gp
    return rolling_gp