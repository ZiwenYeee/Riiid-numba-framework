...
def read_enc_data(path = './'):
    ...
    df_enc2 = pd.read_csv("./content_id_task_set_distance.csv")
...
def content_emb_dict_init_(train, path = './', question_path = './questions.csv'):
    ...
    new_content_dict = np.zeros((content_emb_dict.shape[0], 3))
    new_content_dict[:, 0] = content_emb_dict[:,9]/content_emb_dict[:, 11]
    new_content_dict[:, 1] = content_emb_dict[:,33]/content_emb_dict[:, 35]
    new_content_dict[:, 2] = content_emb_dict[:,37]/content_emb_dict[:, 39]
    new_content_cols = ['div_ratio1', 'div_ratio2', 'div_ratio3']
    
    content_emb_dict = np.concatenate((content_emb_dict, new_content_dict), axis = 1)
    emb_cols += new_content_cols
